...
import com.company.Machine.Crampon;
import com.company.Machine.Machinery;
import com.company.Machine.Truck;
...
public class Main {
    ...
    public static void main(String[] args) {
        Main main = new Main();
        main.readFile("src/com/company/machinerys.txt");
        //main.getMac(main.readFile("src/com/company/machinerys.txt"))

    }

    /*public Machinery[] getMac(ArrayList<String> machines){
        Machinery[] machinery = new Machinery[machines.size() / 3];
        int counter = 0;
        for(int i = 0; i < machinery.length; i++){
            if(machinery[i].equals("Markolo")){
                Crampon crampon = new Crampon(machines[i+1], Integer.parseInt(machines[i+2]), Integer.parseInt(machines[i+3]));
                machinery[counter] = crampon;
                counter++;
                i += 3;
            } else if (machinery[i].equals("Teherauto")){
                Truck truck = new Truck(machines[i+1], Integer.parseInt(machines[i+2]), Integer.parseInt(machines[i+3]));
                machinery[counter] = truck;
                counter++;
                i += 3;
            }
        }
...
/*Írj futtatható osztályt csomagon kívül, amely a munkagepek.txt fájl tartalma alapján
 példányosít Markoló és Teherautó objektumokat. A fájl egy sora egy Markolót és egy
 Teherautót ír le: a „Markolo" szöveg után következik a tipus, a suly és a kanalmeret,
  a „Teherauto" szöveg után pedig a tipus, a suly és a teherbiras egy-egy szóközzel elválasztva.
   Példa:
...
...
Az egyes sorok alapján létrehozott Markolót és Teherautót helyezd el egy HashMap-ben,
 ahol a Markoló a kulcs, a Teherautó pedig az érték. A beolvasást követően határozd meg,
  hogy melyik páros képes a leggyorsabban átrakodni 600 egység homokot 200 egység távolságra
  a Markolo objektumok atrakod() metódusának meghívásával. Az eredményt írd ki a képernyőre.
  Az esetleges ki-/bemeneti hibákat és a Teherauto osztály szallit() metódusa által dobott
...
Hozz létre egy TeherAutoStat osztályt, melyben el tudjuk tárolni, hogy az egyes
Teherau típusok milyen teherbírással rendelkeznek. Pl. ha van két IFA Teherautó objektumunk
különböző teherbírásokkal (pl 100 és 120 -s teherbírással) akkor az IFA-hoz tároljuk el ezeket
 az adatokat. (meg kell találni a helyes adatszerkezetet) Írj egy metodust, mely bemenetnek
 egy Teherauto típust vár (pl. IFA) és visszaadja a hozzá tartozó teherbírásokat!
